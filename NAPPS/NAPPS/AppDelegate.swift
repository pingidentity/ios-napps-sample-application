...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, handleOpenURL url: NSURL) -> Bool {
        ...
        if baseUrl.lowercaseString == app_redirect_uri.lowercaseString {
            ...
            // Kick off the authentication flow (see ProtocolHelper.swift)
            SessionManager.currentSession.inErrorState = false;
            SessionManager.currentSession.isAuthenticated = false;
            handleAuthorizationResponse(url)
        }
        return true
    }
}
